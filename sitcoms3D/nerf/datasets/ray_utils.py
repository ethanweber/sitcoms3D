...
def get_rays_batch_version(directions, c2w):
    """
    Get ray origin and normalized directions in world coordinate for all pixels in one image.
    Reference: https://www.scratchapixel.com/lessons/3d-basic-rendering/
               ray-tracing-generating-camera-rays/standard-coordinate-systems
    ...
    Inputs:
        directions: (N, 3) precomputed ray directions in camera coordinate
    ...
    Outputs:
        ...
        rays_d: (N, 3), the normalized direction of the rays in world coordinate
    """
    # Rotate ray directions from camera coordinate to the world coordinate
    # torch.bmm requires 3D tensors
    rays_d = torch.bmm(c2w[:, :, :3], directions[:, :, None]).view(-1, 3)
    rays_d = rays_d / torch.norm(rays_d, dim=-1, keepdim=True)
    # The origin of all rays is the camera origin in world coordinate
    rays_o = c2w[:, :, 3]
