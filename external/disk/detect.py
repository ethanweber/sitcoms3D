...
class SceneDataset:
    def __init__(self, image_path, crop_size=(None, None), image_list_path=None):
        assert image_list_path is not None
        self.image_path = image_path
        self.crop_size  = crop_size
        file1 = open(image_list_path, 'r')
        lines = file1.readlines()
        self.names = [p.strip().replace("\n", "") for p in lines if p.strip().replace("\n", "").endswith(args.image_extension)]
        file1.close()
...
def extract(dataset, save_path):
    ...
    for bitmaps, images in pbar:
        ...
        for features, image in zip(batched_features.flat, images):
            ...
            # Minor edits for edge case (if there is only one detection)
            if len(mask) == 1:
                if mask.item() == True:
                    keypoints   = kps_img_space.numpy().T
                    descriptors = features.desc.numpy()
                    scores      = features.kp_logp.numpy()
                else:
                    print('no detections?')
            else:
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--image_list_path', type=str, default=None,
        help=('keyframes.txt or keyframes_reg.txt')
    )
    args = parser.parse_args()
    DEV   = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    CPU   = torch.device('cpu')
    dataset = SceneDataset(args.image_path, crop_size=(args.height, args.width), image_list_path=args.image_list_path)
